...
function createRouter(init) {
  ...
  function initialize() {
    unlistenHistory = init.history.listen(
      ({ action: historyAction, location: location$1, delta }) => {
  ...
  function completeNavigation(location$1, newState, { flushSync } = {}) {
    let isActionReload =
      ...
      location$1.state?._isRedirect !== true;
    let actionData;
    ...
    let restoreScrollPosition = isUninterruptedRevalidation
      ? false
      : getSavedScrollPosition(location$1, newState.matches || state.matches);
    ...
    } else if (pendingAction === 'PUSH') init.history.push(location$1, location$1.state);
    else if (pendingAction === 'REPLACE') init.history.replace(location$1, location$1.state);
    let viewTransitionOpts;
    if (pendingAction === 'POP') {
      let priorPaths = appliedViewTransitions.get(state.location.pathname);
      if (priorPaths && priorPaths.has(location$1.pathname))
        viewTransitionOpts = {
          currentLocation: state.location,
          nextLocation: location$1,
        };
      else if (appliedViewTransitions.has(location$1.pathname))
        viewTransitionOpts = {
  ...
  async function startNavigation(historyAction, location$1, opts) {
    ...
    let matches =
      opts?.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW
        ? state.matches
        : matchRoutes(routesToUse, location$1, basename);
    let flushSync = (opts && opts.flushSync) === true;
    if (
      matches &&
      state.initialized &&
      !isRevalidationRequired &&
      isHashChangeOnly(state.location, location$1) &&
  ...
  async function handleAction(
    ...
    interruptActiveLoads();
    updateState(
      { navigation: getSubmittingNavigation(location$1, submission) },
      { flushSync: opts.flushSync === true }
    );
    if (isFogOfWar) {
  ...
  async function startRedirectNavigation(
    ...
    location$1 = normalizeRedirectLocation(
      location$1,
      new URL(request.url),
      basename,
      init.history
  ...
  if (init.unstable_instrumentations)
    router$1 = instrumentClientSideRouter(
      router$1,
...
function createStaticHandler(routes, opts) {
  ...
  async function queryRoute(
    ...
    let match = routeId
      ...
      : getTargetMatch(matches, location$1);
    if (routeId && !match)
      ...
      return await runServerMiddlewarePipeline(
        ...
        async () => {
          return await generateMiddlewareResponse(async (innerRequest) => {
            let processed = handleQueryResult(
              await queryImpl(
                innerRequest,
                location$1,
                matches,
    ...
    try {
      if (isMutationMethod(request.method))
        return await submit(
          request,
          matches,
          routeMatch || getTargetMatch(matches, location$1),
          requestContext,
          dataStrategy,
          skipLoaderErrorBubbling,
          routeMatch != null,
...
function normalizeTo(location$1, matches, basename, to, fromRouteId, relative) {
  ...
  let path = resolveTo(
    ...
    stripBasename(location$1.pathname, basename) || location$1.pathname,
    relative === 'path'
  );
  if (to == null) {
...
Please change the parent <Route path="${parentPath}"> to <Route path="${parentPath === '/' ? '*' : `${parentPath}/*`}">.`
  ...
  if (ENABLE_DEV_WARNINGS) {
    warning(
      ...
      `No routes matched location "${location$1.pathname}${location$1.search}${location$1.hash}" `
    );
    warning(
      matches == null ||
      ...
      `Matched leaf route at location "${location$1.pathname}${location$1.search}${location$1.hash}" does not have an element or Component. This means it will render an <Outlet /> with a null value by default resulting in an "empty" page.`
    );
  }
...
function RouterProvider({
  ...
  let setState = import_react.useCallback(
    (newState, { deletedFetchers, newErrors, flushSync, viewTransitionOpts }) => {
      ...
      let isViewTransitionAvailable =
        router$1.window != null &&
        router$1.window.document != null &&
  ...
  }, [pendingState, renderDfd, router$1.window, unstable_useTransitions, setOptimisticState]);
  import_react.useEffect(() => {
  ...
  let dataRouterContext = import_react.useMemo(
    ...
    [router$1, navigator, basename, onError]
  );
  return /* @__PURE__ */ import_react.createElement(
    import_react.Fragment,
    null,
...
function Await({ children, errorElement, resolve }) {
  ...
  let onError = import_react.useCallback(
    (error, errorInfo) => {
...
async function singleFetchLoaderNavigationStrategy(
    ...
    args.matches.map(async (m, i) =>
      m.resolve(async (handler) => {
        ...
        if (!m.shouldCallHandler(defaultShouldRevalidate)) {
          foundOptOutRoute ||
            (foundOptOutRoute =
              m.shouldRevalidateArgs != null && hasLoader && hasShouldRevalidate === true);
          return;
        }
  ...
  if (
    ((!router$1.state.initialized && router$1.state.navigation.state === 'idle') ||
      routesParams.size === 0) &&
    ...
    return nextMatches.filter((match, index) => {
      ...
      if (match.route.shouldRevalidate) {
        let routeChoice = match.route.shouldRevalidate({
          currentUrl: new URL(
            location$1.pathname + location$1.search + location$1.hash,
            window.origin
          ),
...
function useFogOFWarDiscovery(router$1, manifest, routeModules, ssr, routeDiscovery, isSpaMode) {
  ...
  }, [ssr, isSpaMode, manifest, routeModules, router$1, routeDiscovery]);
}
...
function PrefetchPageLinks({ page, ...linkProps }) {
  ...
  let matches = import_react.useMemo(
    () => matchRoutes(router$1.routes, page, router$1.basename),
    [router$1.routes, page, router$1.basename]
  );
...
function PrefetchPageLinksImpl({ page, matches: nextMatches, ...linkProps }) {
  ...
  let newMatchesForData = import_react.useMemo(
    () => getNewMatchesForLinks(page, nextMatches, matches, manifest, location$1, 'data'),
    [page, nextMatches, matches, manifest, location$1]
  );
  let newMatchesForAssets = import_react.useMemo(
    () => getNewMatchesForLinks(page, nextMatches, matches, manifest, location$1, 'assets'),
    [page, nextMatches, matches, manifest, location$1]
  );
...
${matches
  .map((match, routeIndex) => {
    ...
    return [
      chunks.map((chunk) => `import * as ${chunk.varName} from "${chunk.module}";`).join('\n'),
  ...
  return import_react.useCallback(
    (event) => {
      if (shouldProcessLinkClick(event, target)) {
        event.preventDefault();
        let replace2 =
          replaceProp !== void 0 ? replaceProp : createPath(location$1) === createPath(path);
        let doNavigate = () =>
          navigate(to, {
            replace: replace2,
            state,
...
function useFetcher({ key } = {}) {
  ...
  let reset = import_react.useCallback(
...
function useScrollRestoration({ getKey, storageKey } = {}) {
  ...
  usePageHide(
    import_react.useCallback(() => {
      ...
      window.history.scrollRestoration = 'auto';
    }, [navigation.state, getKey, basename, location$1, matches, storageKey])
  );
  if (typeof document !== 'undefined') {
    ...
    import_react.useLayoutEffect(() => {
      ...
      try {
        ...
        warning(
          false,
          `"${location$1.hash.slice(1)}" is not a decodable element ID. The view will not scroll to it.`
        );
      }
      if (preventScrollReset === true) return;
      window.scrollTo(0, 0);
    }, [location$1, restoreScrollPosition, preventScrollReset]);
  }
}
...
async function singleFetchAction(
  ...
  try {
    ...
    return handleQueryResult(
      await staticHandler.query(handlerRequest, {
...
async function handleResourceRequest(
  ...
  try {
    return handleQueryRouteResult(
      await staticHandler.queryRoute(request, {
        routeId,
        requestContext: loadContext,
        generateMiddlewareResponse: build.future.v8_middleware
          ? async (queryRoute) => {
  ...
  try {
    ...
    while ((read = await reader.read()) && !read.done) {
      ...
      try {
        ...
        writeChunk(
          `Uint8Array.from(atob(${JSON.stringify(btoa(String.fromCodePoint(...chunk)))}), m => m.codePointAt(0))`,
          controller
        );
      }
    }
...
function RSCStaticRouter({ getPayload }) {
  ...
  const router$1 = createStaticRouter(
