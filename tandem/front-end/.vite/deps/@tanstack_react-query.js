...
function hydrate(client, dehydratedState, options) {
  ...
  queries.forEach(({ queryKey, state, queryHash, meta, promise, dehydratedAt }) => {
    ...
    if (
      ...
      query
        .fetch(void 0, { initialPromise: Promise.resolve(promise).then(deserializeData) })
        .catch(noop);
  });
}
        ...
        if (key === 'promise') {
          ...
          if (
            ...
            this.#currentThenable.reject(
              /* @__PURE__ */ new Error('experimental_prefetchInRender feature flag is not enabled')
            );
        }
...
var QueriesObserver = class extends Subscribable {
  ...
  #findMatchingObservers(queries) {
    ...
    queries.forEach((options) => {
      const defaultedOptions = this.#client.defaultQueryOptions(options);
      const observer =
        prevObserversMap.get(defaultedOptions.queryHash)?.shift() ??
  ...
  #notify() {
    if (this.hasListeners()) {
      ...
      if (previousResult !== this.#combineResult(newTracked, this.#options?.combine))
        notifyManager.batch(() => {
          this.listeners.forEach((listener) => {
...
function useQueries({ queries, ...options }, queryClient) {
  ...
  const defaultedQueries = import_react.useMemo(
    () =>
      queries.map((opts) => {
  ...
  defaultedQueries.forEach((queryOptions$1) => {
    ...
    ensurePreventErrorBoundaryRetry(
      queryOptions$1,
      errorResetBoundary,
      client.getQueryCache().get(queryOptions$1.queryHash)
    );
  });
  useClearResetErrorBoundary(errorResetBoundary);
  const [observer] = import_react.useState(
