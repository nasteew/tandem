...
function assert(bool, actual, expected, operator, defaultMessage, userMessage) {
  if (!bool)
    throw userMessage instanceof Error
      ? userMessage
      : new AssertionError(userMessage || defaultMessage, actual, expected, operator, !userMessage);
}
...
function stringify(values$1, options) {
  ...
  return (values$1[values$1.length - 1] === '' ? [...values$1, ''] : values$1)
...
function create(definition$1) {
  ...
  for (const [property, value] of Object.entries(definition$1.properties)) {
    const info = new DefinedInfo(
      property,
      definition$1.transform(definition$1.attributes || {}, property),
      value,
      definition$1.space
    );
    if (definition$1.mustUseProperty && definition$1.mustUseProperty.includes(property))
...
var require_cjs$2 = /* @__PURE__ */ __commonJS({
...
var require_cjs$1 = /* @__PURE__ */ __commonJS({
...
var require_utilities = /* @__PURE__ */ __commonJS({
...
function point$2(type) {
  ...
  function point$3(node$1) {
    ...
    if (
      typeof point$4.line === 'number' &&
      point$4.line > 0 &&
      typeof point$4.column === 'number' &&
      point$4.column > 0
    )
      return {
        line: point$4.line,
...
function one$1(state, node$1, key) {
  ...
  if (node$1.type === 'mdxFlowExpression' || node$1.type === 'mdxTextExpression')
    return mdxExpression(state, node$1);
  if (node$1.type === 'mdxJsxFlowElement' || node$1.type === 'mdxJsxTextElement')
    return mdxJsxElement(state, node$1, key);
...
function element$1(state, node$1, key) {
  ...
  if (tableElements.has(node$1.tagName))
...
function mdxEsm(state, node$1) {
  if (node$1.data && node$1.data.estree && state.evaluater)
    return state.evaluater.evaluateProgram(node$1.data.estree);
  crashEstree(state, node$1.position);
}
/**
 * Handle MDX JSX.
...
function mdxJsxElement(state, node$1, key) {
  ...
  state.ancestors.push(node$1);
  const type =
    node$1.name === null ? state.Fragment : findComponentFromName(state, node$1.name, true);
...
function createElementProps(state, node$1) {
  ...
  /** @type {string} */
  let prop;
  for (prop in node$1.properties)
    if (prop !== 'children' && own$3.call(node$1.properties, prop)) {
      ...
      if (result) {
        ...
        if (
          ...
          key === 'align' &&
          typeof value === 'string' &&
          tableCellElement.has(node$1.tagName)
        )
          alignValue = value;
...
function createJsxElementProps(state, node$1) {
  /** @type {Props} */
  const props = {};
  for (const attribute of node$1.attributes)
...
function transformStylesToCssCasing(domCasing) {
  ...
  for (from in domCasing)
    if (own$3.call(domCasing, from)) cssCasing[transformStyleToCssCasing(from)] = domCasing[from];
  return cssCasing;
}
/**
...
function all(values$1, includeImageAlt, includeHtml) {
  ...
  const result = [];
  let index$2 = -1;
  while (++index$2 < values$1.length)
    result[index$2] = one(values$1[index$2], includeImageAlt, includeHtml);
  return result.join('');
}
/**
...
function constructs(existing, list$2) {
  ...
  while (++index$2 < list$2.length)
    (list$2[index$2].add === 'after' ? existing : before).push(list$2[index$2]);
...
function decodeNumericCharacterReference(value, base) {
  ...
  if (
    code$1 < codes.ht ||
    code$1 === codes.vt ||
    (code$1 > codes.cr && code$1 < codes.space) ||
    (code$1 > codes.tilde && code$1 < 160) ||
    (code$1 > 55295 && code$1 < 57344) ||
    (code$1 > 64975 && code$1 < 65008) ||
    (code$1 & 65535) === 65535 ||
    (code$1 & 65535) === 65534 ||
    code$1 > 1114111
  )
...
function normalizeUri(value) {
  ...
  while (++index$2 < value.length) {
    ...
    let replace = '';
    if (
      code$1 === codes.percentSign &&
      asciiAlphanumeric(value.charCodeAt(index$2 + 1)) &&
      asciiAlphanumeric(value.charCodeAt(index$2 + 2))
    )
      skip = 2;
    else if (code$1 < 128) {
      if (!/[!#$&-;=?-Z_a-z~]/.test(String.fromCharCode(code$1)))
        replace = String.fromCharCode(code$1);
...
function initializeContent(effects) {
  ...
  function paragraphInitial(code$1) {
    ok(
      code$1 !== codes.eof && !markdownLineEnding(code$1),
...
function initializeDocument(effects) {
  ...
  function documentContinue(code$1) {
    ...
    if (self$1.containerState._closeFlow) {
      ...
      while (indexBeforeFlow--)
        if (
          self$1.events[indexBeforeFlow][0] === 'exit' &&
          self$1.events[indexBeforeFlow][1].type === types.chunkFlow
        ) {
  ...
  function writeToChild(token, endOfFile) {
    ...
    if (self$1.parser.lazy[token.start.line]) {
      ...
      while (index$2--)
        if (
          childFlow.events[index$2][1].start.offset < lineStartOffset &&
          (!childFlow.events[index$2][1].end ||
            childFlow.events[index$2][1].end.offset > lineStartOffset)
        )
          return;
      const indexBeforeExits = self$1.events.length;
      let indexBeforeFlow = indexBeforeExits;
...
function classifyCharacter(code$1) {
  if (code$1 === codes.eof || markdownLineEndingOrSpace(code$1) || unicodeWhitespace(code$1))
...
function resolveAllAttention(events, context) {
  ...
  while (++index$2 < events.length)
    if (
      events[index$2][0] === 'enter' &&
      events[index$2][1].type === 'attentionSequence' &&
      events[index$2][1]._close
    ) {
      open = index$2;
      while (open--)
        if (
          ...
          context.sliceSerialize(events[open][1]).charCodeAt(0) ===
            context.sliceSerialize(events[index$2][1]).charCodeAt(0)
        ) {
          if (
            (events[open][1]._close || events[index$2][1]._open) &&
            (events[index$2][1].end.offset - events[index$2][1].start.offset) % 3 &&
            !(
              (events[open][1].end.offset -
                events[open][1].start.offset +
                events[index$2][1].end.offset -
                events[index$2][1].start.offset) %
              3
            )
          )
            continue;
          use =
            ...
            events[index$2][1].end.offset - events[index$2][1].start.offset > 1
              ? 2
              : 1;
          ...
          if (events[index$2][1].end.offset - events[index$2][1].start.offset) {
            offset = 2;
            nextEvents = push(nextEvents, [
              ['enter', events[index$2][1], context],
              ['exit', events[index$2][1], context],
            ]);
          } else offset = 0;
  ...
  index$2 = -1;
  while (++index$2 < events.length)
    if (events[index$2][1].type === 'attentionSequence') events[index$2][1].type = 'data';
  return events;
}
/**
 * @this {TokenizeContext}
...
function tokenizeAttention(effects, ok$2) {
  ...
  function inside(code$1) {
    ...
    const open =
      ...
      attentionMarkers$1.includes(code$1);
    const close =
      !before ||
      (before === constants.characterGroupPunctuation && after) ||
      attentionMarkers$1.includes(previous$1);
...
function tokenizeAutolink(effects, ok$2, nok) {
  ...
  function schemeOrEmailAtext(code$1) {
    if (
      code$1 === codes.plusSign ||
      code$1 === codes.dash ||
      code$1 === codes.dot ||
      asciiAlphanumeric(code$1)
    ) {
      size = 1;
  ...
  function emailValue(code$1) {
    if (
      (code$1 === codes.dash || asciiAlphanumeric(code$1)) &&
...
function tokenizeBlankLine(effects, ok$2, nok) {
   ...
   */
  function start(code$1) {
    return markdownSpace(code$1)
      ? factorySpace(effects, after, types.linePrefix)(code$1)
      : after(code$1);
  }
  /**
  ...
  function contStart(code$1) {
    if (markdownSpace(code$1)) {
      ...
      return factorySpace(
        ...
        contBefore,
        types.linePrefix,
        self$1.parser.constructs.disable.null.includes('codeIndented') ? void 0 : constants.tabSize
      )(code$1);
    }
...
function tokenizeCharacterReference(effects, ok$2, nok) {
  ...
  function value(code$1) {
    if (code$1 === codes.semicolon && size) {
      ...
      if (
        test === asciiAlphanumeric &&
        !decodeNamedCharacterReference(self$1.sliceSerialize(token))
      )
        return nok(code$1);
      effects.enter(types.characterReferenceMarker);
      effects.consume(code$1);
...
function tokenizeCodeFenced(effects, ok$2, nok) {
  ...
  function sequenceOpen(code$1) {
    ...
    return markdownSpace(code$1)
      ? factorySpace(effects, infoBefore, types.whitespace)(code$1)
      : infoBefore(code$1);
  }
  /**
  ...
  function infoBefore(code$1) {
    if (code$1 === codes.eof || markdownLineEnding(code$1)) {
      ...
      return self$1.interrupt
        ? ok$2(code$1)
        : effects.check(nonLazyContinuation, atNonLazyBreak, after)(code$1);
    }
  ...
  function contentStart(code$1) {
    return initialPrefix > 0 && markdownSpace(code$1)
      ? factorySpace(effects, beforeContentChunk, types.linePrefix, initialPrefix + 1)(code$1)
      : beforeContentChunk(code$1);
  }
  /**
  ...
  function beforeContentChunk(code$1) {
    if (code$1 === codes.eof || markdownLineEnding(code$1))
      return effects.check(nonLazyContinuation, atNonLazyBreak, after)(code$1);
  ...
  function tokenizeCloseStart(effects$1, ok$3, nok$1) {
    ...
    function start$1(code$1) {
      ...
      return markdownSpace(code$1)
        ? factorySpace(
            ...
            beforeSequenceClose,
            types.linePrefix,
            self$1.parser.constructs.disable.null.includes('codeIndented')
              ? void 0
              : constants.tabSize
          )(code$1)
        : beforeSequenceClose(code$1);
    }
    /**
    ...
    function sequenceClose(code$1) {
      ...
      if (size >= sizeOpen) {
        ...
        return markdownSpace(code$1)
          ? factorySpace(effects$1, sequenceCloseAfter, types.whitespace)(code$1)
          : sequenceCloseAfter(code$1);
      }
      return nok$1(code$1);
    }
    /**
...
function tokenizeFurtherStart(effects, ok$2, nok) {
  ...
  function afterPrefix(code$1) {
    ...
    return tail &&
      ...
      tail[2].sliceSerialize(tail[1], true).length >= constants.tabSize
      ? ok$2(code$1)
      : markdownLineEnding(code$1)
        ? furtherStart$1(code$1)
        : nok(code$1);
  }
}
...
function resolveCodeText(events) {
  ...
  if (
    ...
    while (++index$2 < tailExitIndex)
      if (events[index$2][1].type === types.codeTextData) {
  ...
  while (++index$2 <= tailExitIndex)
    if (enter === void 0) {
      if (index$2 !== tailExitIndex && events[index$2][1].type !== types.lineEnding)
        enter = index$2;
...
var SpliceBuffer = class {
   ...
   */
  get(index$2) {
    if (index$2 < 0 || index$2 >= this.left.length + this.right.length)
...
function subtokenize(eventsArray) {
  ...
  while (++index$2 < events.length) {
    ...
    if (
      ...
      events.get(index$2 - 1)[1].type === types.listItemPrefix
    ) {
...
function subcontent(events, eventIndex) {
  ...
  current = token;
  while (++index$2 < childEvents.length)
    if (
      childEvents[index$2][0] === 'exit' &&
      childEvents[index$2 - 1][0] === 'enter' &&
      childEvents[index$2][1].type === childEvents[index$2 - 1][1].type &&
      childEvents[index$2][1].start.line !== childEvents[index$2][1].end.line
    ) {
...
function tokenizeContent(effects, ok$2) {
  ...
  function chunkInside(code$1) {
    ...
    if (markdownLineEnding(code$1))
      return effects.check(continuationConstruct, contentContinue, contentEnd)(code$1);
    effects.consume(code$1);
    return chunkInside;
  }
  /**
...
function factoryDestination(
  ...
  function enclosed(code$1) {
    ...
    if (code$1 === codes.eof || code$1 === codes.lessThan || markdownLineEnding(code$1))
      return nok(code$1);
    effects.consume(code$1);
  ...
  function raw(code$1) {
    if (
      ...
      (code$1 === codes.eof ||
        code$1 === codes.rightParenthesis ||
        markdownLineEndingOrSpace(code$1))
    ) {
      effects.exit(types.chunkString);
      effects.exit(stringType);
...
function factoryLabel(effects, ok$2, nok, type, markerType, stringType) {
  ...
  function atBreak(code$1) {
    if (
      ...
      code$1 === codes.eof ||
      code$1 === codes.leftSquareBracket ||
      (code$1 === codes.rightSquareBracket && !seen) ||
      (code$1 === codes.caret && !size && '_hiddenFootnoteSupport' in self$1.parser.constructs)
    )
      return nok(code$1);
  ...
  function labelInside(code$1) {
    if (
      code$1 === codes.eof ||
      code$1 === codes.leftSquareBracket ||
      code$1 === codes.rightSquareBracket ||
      markdownLineEnding(code$1) ||
      size++ > constants.linkReferenceSizeMax
    ) {
      effects.exit(types.chunkString);
...
function factoryTitle(effects, ok$2, nok, type, markerType, stringType) {
  ...
  function start(code$1) {
    if (
      code$1 === codes.quotationMark ||
      code$1 === codes.apostrophe ||
      code$1 === codes.leftParenthesis
    ) {
      effects.enter(type);
      effects.enter(markerType);
...
function factoryWhitespace(effects, ok$2) {
  ...
  function start(code$1) {
    ...
    if (markdownSpace(code$1))
      return factorySpace(effects, start, seen ? types.linePrefix : types.lineSuffix)(code$1);
    return ok$2(code$1);
  }
}
...
function tokenizeDefinition(effects, ok$2, nok) {
  ...
  function markerAfter(code$1) {
    return markdownLineEndingOrSpace(code$1)
      ? factoryWhitespace(effects, destinationBefore)(code$1)
      : destinationBefore(code$1);
  }
  /**
   * Before destination.
   *
  ...
  function after(code$1) {
    return markdownSpace(code$1)
      ? factorySpace(effects, afterWhitespace, types.whitespace)(code$1)
      : afterWhitespace(code$1);
  }
  /**
...
function tokenizeTitleBefore(effects, ok$2, nok) {
  ...
  function titleBefore$1(code$1) {
    return markdownLineEndingOrSpace(code$1)
      ? factoryWhitespace(effects, beforeMarker)(code$1)
      : nok(code$1);
  }
  /**
   * At title.
  ...
  function titleAfter(code$1) {
    return markdownSpace(code$1)
      ? factorySpace(effects, titleAfterOptionalWhitespace, types.whitespace)(code$1)
      : titleAfterOptionalWhitespace(code$1);
  }
  /**
...
function resolveHeadingAtx(events, context) {
  ...
  if (contentEnd > contentStart) {
    ...
    splice(events, contentStart, contentEnd - contentStart + 1, [
      ['enter', content$2, context],
      ['enter', text$4, context],
      ['exit', text$4, context],
      ['exit', content$2, context],
    ]);
  }
  return events;
}
...
function resolveToHtmlFlow(events) {
  ...
  while (index$2--)
    if (events[index$2][0] === 'enter' && events[index$2][1].type === types.htmlFlow) break;
...
function tokenizeHtmlFlow(effects, ok$2, nok) {
  ...
  function tagName(code$1) {
    if (
      ...
      return self$1.interrupt && !self$1.parser.lazy[self$1.now().line]
        ? nok(code$1)
        : closingTag
          ? completeClosingTagAfter(code$1)
          : completeAttributeNameBefore(code$1);
    }
  ...
  function completeAttributeValueQuotedAfter(code$1) {
    if (code$1 === codes.slash || code$1 === codes.greaterThan || markdownSpace(code$1))
      return completeAttributeNameBefore(code$1);
    return nok(code$1);
  }
  /**
...
function tokenizeHtmlText(effects, ok$2, nok) {
  ...
  function commentEnd(code$1) {
    return code$1 === codes.greaterThan
      ? end(code$1)
      : code$1 === codes.dash
        ? commentClose(code$1)
        : comment(code$1);
  ...
  function tagOpen(code$1) {
    ...
    if (code$1 === codes.slash || code$1 === codes.greaterThan || markdownLineEndingOrSpace(code$1))
...
function resolveToLabelEnd(events, context) {
  ...
  media = push(media, [['enter', text$4, context]]);
...
function tokenizeLabelEnd(effects, ok$2, nok) {
  ...
  while (index$2--)
    if (
      (self$1.events[index$2][1].type === types.labelImage ||
        self$1.events[index$2][1].type === types.labelLink) &&
      !self$1.events[index$2][1]._balanced
    ) {
  ...
  function start(code$1) {
    ...
    defined = self$1.parser.defined.includes(
      normalizeIdentifier(
        self$1.sliceSerialize({
   ...
   */
  function after(code$1) {
    if (code$1 === codes.leftParenthesis)
...
function tokenizeResource(effects, ok$2, nok) {
  ...
  function resourceBefore(code$1) {
    return markdownLineEndingOrSpace(code$1)
      ? factoryWhitespace(effects, resourceOpen)(code$1)
      : resourceOpen(code$1);
  }
  /**
  ...
  function resourceDestinationAfter(code$1) {
    return markdownLineEndingOrSpace(code$1)
      ? factoryWhitespace(effects, resourceBetween)(code$1)
      : resourceEnd(code$1);
  }
  /**
  ...
  function resourceTitleAfter(code$1) {
    return markdownLineEndingOrSpace(code$1)
      ? factoryWhitespace(effects, resourceEnd)(code$1)
      : resourceEnd(code$1);
  }
  /**
   * In resource, at `)`.
...
function tokenizeLabelStartImage(effects, ok$2, nok) {
  ...
  function after(code$1) {
    /* c8 ignore next 3 */
    return code$1 === codes.caret && '_hiddenFootnoteSupport' in self$1.parser.constructs
      ? nok(code$1)
      : ok$2(code$1);
  }
}
...
function tokenizeThematicBreak(effects, ok$2, nok) {
  ...
  function before(code$1) {
    ok(
      code$1 === codes.asterisk || code$1 === codes.dash || code$1 === codes.underscore,
      'expected `*`, `-`, or `_`'
    );
    marker = code$1;
    return atBreak(code$1);
  ...
  function sequence(code$1) {
    ...
    return markdownSpace(code$1)
      ? factorySpace(effects, atBreak, types.whitespace)(code$1)
      : atBreak(code$1);
  }
}
...
function tokenizeListStart(effects, ok$2, nok) {
  ...
  function start(code$1) {
    ...
    const kind =
      self$1.containerState.type ||
      (code$1 === codes.asterisk || code$1 === codes.plusSign || code$1 === codes.dash
        ? types.listUnordered
        : types.listOrdered);
    if (
      kind === types.listUnordered
        ? !self$1.containerState.marker || code$1 === self$1.containerState.marker
        : asciiDigit(code$1)
    ) {
      ...
      if (kind === types.listUnordered) {
        ...
        return code$1 === codes.asterisk || code$1 === codes.dash
          ? effects.check(thematicBreak$1, nok, atMarker)(code$1)
          : atMarker(code$1);
      }
  ...
  function inside(code$1) {
    ...
    if (
      (!self$1.interrupt || size < 2) &&
      (self$1.containerState.marker
        ? code$1 === self$1.containerState.marker
        : code$1 === codes.rightParenthesis || code$1 === codes.dot)
    ) {
  ...
  function endOfPrefix(code$1) {
    ok(self$1.containerState, 'expected state');
    self$1.containerState.size =
      initialSize + self$1.sliceSerialize(effects.exit(types.listItemPrefix), true).length;
    return ok$2(code$1);
  }
}
/**
...
function tokenizeListContinuation(effects, ok$2, nok) {
  ...
  function onBlank(code$1) {
    ...
    self$1.containerState.furtherBlankLines =
      self$1.containerState.furtherBlankLines || self$1.containerState.initialBlankLine;
    return factorySpace(
      effects,
      ok$2,
      types.listItemIndent,
      self$1.containerState.size + 1
    )(code$1);
  }
  /** @type {State} */
...
function tokenizeIndent(effects, ok$2, nok) {
  ...
  function afterPrefix(code$1) {
    ...
    return tail &&
      ...
      tail[2].sliceSerialize(tail[1], true).length === self$1.containerState.size
      ? ok$2(code$1)
      : nok(code$1);
  }
}
...
function tokenizeListItemPrefixWhitespace(effects, ok$2, nok) {
  ...
  return factorySpace(
    ...
    self$1.parser.constructs.disable.null.includes('codeIndented') ? void 0 : constants.tabSize + 1
  );
  /** @type {State} */
  function afterPrefix(code$1) {
    const tail = self$1.events[self$1.events.length - 1];
    return !markdownSpace(code$1) && tail && tail[1].type === types.listItemPrefixWhitespace
      ? ok$2(code$1)
      : nok(code$1);
  }
}
...
function resolveToSetextUnderline(events, context) {
  ...
  if (definition$1) {
    events.splice(text$4, 0, ['enter', heading$1, context]);
    events.splice(definition$1 + 1, 0, ['exit', events[content$2][1], context]);
    events[content$2][1].end = { ...events[definition$1][1].end };
  } else events[content$2][1] = heading$1;
  events.push(['exit', heading$1, context]);
  return events;
}
/**
 * @this {TokenizeContext}
...
function tokenizeSetextUnderline(effects, ok$2, nok) {
  ...
  function start(code$1) {
    ...
    while (index$2--)
      if (
        self$1.events[index$2][1].type !== types.lineEnding &&
        self$1.events[index$2][1].type !== types.linePrefix &&
        self$1.events[index$2][1].type !== types.content
      ) {
  ...
  function inside(code$1) {
    ...
    return markdownSpace(code$1)
      ? factorySpace(effects, after, types.lineSuffix)(code$1)
      : after(code$1);
  }
  /**
...
function resolveAllLineSuffixes(events, context) {
  ...
  while (++eventIndex <= events.length)
    if (
      (eventIndex === events.length || events[eventIndex][1].type === types.lineEnding) &&
      ...
      if (size) {
        const token = {
          type:
            eventIndex === events.length || tabs || size < constants.hardBreakPrefixSizeMin
              ? types.lineSuffix
...
var require_common = /* @__PURE__ */ __commonJS({
    ...
    function setup(env$1) {
      ...
      function enable(namespaces) {
        ...
        for (const ns of split)
          if (ns[0] === '-') createDebug$1.skips.push(ns.slice(1));
      ...
      function matchesTemplate(search$1, template) {
        ...
        let starIndex = -1;
        let matchIndex = 0;
        while (searchIndex < search$1.length)
          if (
            templateIndex < template.length &&
            (template[templateIndex] === search$1[searchIndex] || template[templateIndex] === '*')
          )
      ...
      function disable$1() {
        const namespaces = [
          ...createDebug$1.names,
          ...createDebug$1.skips.map((namespace) => '-' + namespace),
        ].join(',');
  ...
  function main() {
    ...
    while (point$3._index < chunks.length) {
      ...
      if (typeof chunk === 'string') {
        ...
        while (point$3._index === chunkIndex && point$3._bufferIndex < chunk.length)
          go(chunk.charCodeAt(point$3._bufferIndex));
      } else go(chunk);
    }
  }
  /**
      ...
      function handleMapOfConstructs(map) {
        ...
        function start(code$1) {
          ...
          return handleListOfConstructs([
            ...
            ...(Array.isArray(all$1) ? all$1 : all$1 ? [all$1] : []),
          ])(code$1);
        }
      }
      /**
  ...
  function addResult(construct, from$1) {
    ...
    if (construct.resolve)
      splice(
        context.events,
        from$1,
        context.events.length - from$1,
        construct.resolve(context.events.slice(from$1), context)
      );
...
function compiler(options) {
  ...
  function compile(events) {
    ...
    while (++index$2 < events.length)
      if (
        events[index$2][1].type === types.listOrdered ||
        events[index$2][1].type === types.listUnordered
      )
        if (events[index$2][0] === 'enter') listStack.push(index$2);
        else {
    ...
    while (++index$2 < events.length) {
      ...
      if (own$2.call(handler, events[index$2][1].type))
        handler[events[index$2][1].type].call(
          Object.assign({ sliceSerialize: events[index$2][2].sliceSerialize }, context),
          events[index$2][1]
        );
    }
    ...
    while (++index$2 <= length) {
      ...
      switch (event[1].type) {
        ...
        case types.lineEndingBlank:
          if (event[0] === 'enter') {
            if (listItem$2 && !atMarker && !containerBalance && !firstBlankLineIndex)
              firstBlankLineIndex = index$2;
            atMarker = void 0;
          }
          break;
      ...
      if (
        ...
        if (listItem$2) {
          ...
          if (firstBlankLineIndex && (!lineIndex || firstBlankLineIndex < lineIndex))
            listItem$2._spread = true;
          listItem$2.end = Object.assign({}, lineIndex ? events[lineIndex][1].start : event[1].end);
          events.splice(lineIndex || index$2, 0, ['exit', listItem$2, event[2]]);
          index$2++;
          length++;
        }
        if (event[1].type === types.listItemPrefix) {
          ...
          listItem$2 = item;
          events.splice(index$2, 0, ['enter', item, event[2]]);
          index$2++;
          length++;
  ...
  function onexitreferencestring(token) {
    ...
    ok(node$1, 'expected node on stack');
    ok(
      node$1.type === 'image' || node$1.type === 'link',
...
function imageReference(state, node$1) {
  ...
  if (definition$1.title !== null && definition$1.title !== void 0)
    properties.title = definition$1.title;
  /** @type {Element} */
  const result = {
    type: 'element',
    tagName: 'img',
...
function listItem(state, node$1, parent) {
  ...
  if (typeof node$1.checked === 'boolean') {
    ...
    if (paragraph$1.children.length > 0)
  ...
  while (++index$2 < results.length) {
    ...
    if (loose || index$2 !== 0 || child.type !== 'element' || child.tagName !== 'p')
      children.push({
        type: 'text',
        value: '\n',
      });
...
var deserializer = ($, _) => {
  ...
  const unpair = (index$2) => {
    ...
    switch (type) {
      ...
      case DATE:
        return as(new Date(value), index$2);
      case REGEXP: {
      ...
      case BIGINT:
        return as(BigInt(value), index$2);
      case 'BigInt':
        return as(Object(BigInt(value)), index$2);
      case 'ArrayBuffer':
  ...
  function check(value, index$2, parent) {
    return Boolean(
      ...
      testFunction.call(
        this,
        value,
        typeof index$2 === 'number' ? index$2 : void 0,
        parent || void 0
      )
    );
  }
      ...
      const name$1 =
        typeof value.tagName === 'string'
          ...
          : typeof value.name === 'string'
            ? value.name
            : void 0;
      Object.defineProperty(visit$1, 'name', {
        value: 'node (' + color(node$1.type + (name$1 ? '<' + name$1 + '>' : '')) + ')',
      });
    }
    return visit$1;
    function visit$1() {
...
function defaultUnknownHandler(state, node$1) {
  ...
  const result =
    'value' in node$1 && !(own$1.call(data, 'hProperties') || own$1.call(data, 'hChildren'))
      ? {
          type: 'text',
          value: node$1.value,
        }
...
var require_extend = /* @__PURE__ */ __commonJS({
...
function trough() {
  ...
  function run(...values$1) {
    ...
    function next(error, ...output) {
      ...
      while (++index$2 < values$1.length)
        if (output[index$2] === null || output[index$2] === void 0)
          output[index$2] = values$1[index$2];
      values$1 = output;
...
function basename(path, extname$1) {
  if (extname$1 !== void 0 && typeof extname$1 !== 'string')
  ...
  if (extname$1 === void 0 || extname$1.length === 0 || extname$1.length > path.length) {
    while (index$2--)
      if (path.codePointAt(index$2) === 47) {
        if (seenNonSlash) {
          start = index$2 + 1;
          break;
        }
  ...
  while (index$2--)
    if (path.codePointAt(index$2) === 47) {
      ...
      if (extnameIndex > -1)
        if (path.codePointAt(index$2) === extname$1.codePointAt(extnameIndex--)) {
...
function join(...segments) {
  ...
  while (++index$2 < segments.length) {
    ...
    if (segments[index$2])
      joined = joined === void 0 ? segments[index$2] : joined + '/' + segments[index$2];
  }
...
function normalizeString(path, allowAboveRoot) {
  ...
  while (++index$2 <= path.length) {
    ...
    if (code$1 === 47) {
      if (lastSlash === index$2 - 1 || dots === 1) {
      } else if (lastSlash !== index$2 - 1 && dots === 2) {
        if (
          result.length < 2 ||
          lastSegmentLength !== 2 ||
...
function getPathFromURLPosix(url) {
  ...
  let index$2 = -1;
  while (++index$2 < pathname.length)
    if (pathname.codePointAt(index$2) === 37 && pathname.codePointAt(index$2 + 1) === 50) {
...
var VFile = class {
  ...
  constructor(value) {
    ...
    while (++index$2 < order.length) {
      ...
      if (field$1 in options && options[field$1] !== void 0 && options[field$1] !== null)
        this[field$1] = field$1 === 'history' ? [...options[field$1]] : options[field$1];
    }
    /** @type {string} */
    let field;
...
function assertPart(part, name$1) {
  if (part && part.includes(minpath.sep))
    throw new Error('`' + name$1 + '` cannot be a path: did not expect `' + minpath.sep + '`');
}
/**
...
var Processor = class Processor extends CallableInstance {
  ...
  use(value, ...parameters) {
    ...
    function addPlugin(plugin, parameters$1) {
      let index$2 = -1;
      let entryIndex = -1;
      while (++index$2 < attachers.length)
        if (attachers[index$2][0] === plugin) {
          entryIndex = index$2;
          break;
        }
...
function assertCompiler(name$1, value) {
  if (typeof value !== 'function')
    throw new TypeError('Cannot `' + name$1 + '` without `compiler`');
}
/**
...
function assertNode(node$1) {
  if (!isPlainObject(node$1) || typeof node$1.type !== 'string')
    throw new TypeError('Expected node, got `' + node$1 + '`');
}
/**
...
function assertDone(name$1, asyncName, complete) {
  if (!complete)
    throw new Error('`' + name$1 + '` finished async. Use `' + asyncName + '` instead');
}
/**
  ...
  function transform(node$1, index$2, parent) {
    ...
    if (node$1.type === 'element') {
      ...
      for (key in urlAttributes)
        if (Object.hasOwn(urlAttributes, key) && Object.hasOwn(node$1.properties, key)) {
          const value = node$1.properties[key];
          const test = urlAttributes[key];
          if (test === null || test.includes(node$1.tagName))
            node$1.properties[key] = urlTransform(String(value || ''), key, node$1);
        }
    }
    if (node$1.type === 'element') {
      let remove = allowedElements
        ? !allowedElements.includes(node$1.tagName)
        : disallowedElements
          ? disallowedElements.includes(node$1.tagName)
          : false;
      if (!remove && allowElement && typeof index$2 === 'number')
        remove = !allowElement(node$1, index$2, parent);
      if (remove && parent && typeof index$2 === 'number') {
        if (unwrapDisallowed && node$1.children)
          parent.children.splice(index$2, 1, ...node$1.children);
