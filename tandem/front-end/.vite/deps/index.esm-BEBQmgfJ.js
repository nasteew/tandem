...
function cloneObject(data) {
  ...
  for (const key in data)
    if (Object.prototype.hasOwnProperty.call(data, key)) copy[key] = cloneObject(data[key]);
  return copy;
}
...
var get = (object, path, defaultValue) => {
  ...
  const result = (isKey(path) ? [path] : stringToPath(path)).reduce(
    (result$1, key) => (isNullOrUndefined(result$1) ? result$1 : result$1[key]),
    object
  );
...
function Form(props) {
  ...
  return render
    ? import_react.createElement(import_react.Fragment, null, render({ submit }))
...
var validateField = async (
  ...
  if (
    isFieldArray
      ? !Array.isArray(inputValue) || !inputValue.length
      : required &&
        ((!isRadioOrCheckbox$1 && (isEmpty || isNullOrUndefined(inputValue))) ||
...
function createFormControl(props = {}) {
  ...
  const _subscribe = (props$1) =>
    _subjects.state.subscribe({
      next: (formState) => {
        if (
          shouldSubscribeByName(props$1.name, formState.name, props$1.exact) &&
          shouldRenderFormState(
            formState,
            props$1.formState || _proxyFormState,
            _setFormState,
            props$1.reRenderRoot
          )
        )
          props$1.callback({
            values: { ..._formValues },
...
export {
  ...
  useFormContext as p,
  FormProvider as r,
  createFormControl as s,
  Controller as t,
  useController as u,
};
//# sourceMappingURL=index.esm-BEBQmgfJ.js.map
