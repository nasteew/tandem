...
var isPlainObject = (val) => {
  ...
  return (
    (prototype$1 === null ||
      prototype$1 === Object.prototype ||
      Object.getPrototypeOf(prototype$1) === null) &&
    !(toStringTag in val) &&
    !(iterator in val)
  );
};
...
var hasOwnProperty = (
  ...
  (obj, prop) =>
    hasOwnProperty$1.call(obj, prop)
)(Object.prototype);
/**
...
function buildURL(url, params, options) {
  ...
  if (serializeFn) serializedParams = serializeFn(params, _options);
    ...
    serializedParams = utils_default.isURLSearchParams(params)
      ? params.toString()
      : new AxiosURLSearchParams_default(params, _options).toString(_encode);
  if (serializedParams) {
...
function formDataToJSON(formData) {
  function buildPath(path, value, target, index) {
    ...
    if (buildPath(path, value, target[name], index) && utils_default.isArray(target[name]))
  ...
  transformRequest: [
    function transformRequest(data, headers) {
      ...
      if (isObjectPayload) {
        ...
        if (
          (isFileList$1 = utils_default.isFileList(data)) ||
          contentType.indexOf('multipart/form-data') > -1
        ) {
          const _FormData = this.env && this.env.FormData;
          return toFormData_default(
            isFileList$1 ? { 'files[]': data } : data,
  ...
  set(header, valueOrRewrite, rewrite) {
    ...
    function setHeader(_value, _header, _rewrite) {
      ...
      if (
        ...
        self$1[key] === void 0 ||
        _rewrite === true ||
        (_rewrite === void 0 && self$1[key] !== false)
      )
...
var isURLSameOrigin_default = platform_default.hasStandardBrowserEnv
  ? ((origin$1, isMSIE) => (url) => {
      ...
      return (
        origin$1.protocol === url.protocol &&
        origin$1.host === url.host &&
        (isMSIE || origin$1.port === url.port)
      );
    })(
...
function mergeConfig$1(config1, config2) {
  ...
  utils_default.forEach(
    ...
    function computeConfigValue(prop) {
      ...
      const merge$1 = utils_default.hasOwnProp(mergeMap, prop)
        ? mergeMap[prop]
        : mergeDeepProperties;
      const configValue = merge$1(config1[prop], config2[prop], prop);
      (utils_default.isUndefined(configValue) && merge$1 !== mergeDirectKeys) ||
  ...
  if (timeout || length) {
    ...
    let timer =
      ...
      setTimeout(() => {
        timer = null;
        onabort(
          new AxiosError_default(`timeout of ${timeout}ms exceeded`, AxiosError_default.ETIMEDOUT)
        );
      }, timeout);
    const unsubscribe = () => {
      if (signals) {
        ...
        signals.forEach((signal$1) => {
          signal$1.unsubscribe
            ? signal$1.unsubscribe(onabort)
 ...
 * @returns {boolean}
 */
var isResolvedHandle = (adapter$1) =>
  utils_default.isFunction(adapter$1) || adapter$1 === null || adapter$1 === false;
/**
...
function getAdapter$1(adapters, config) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (adapter$1 && (utils_default.isFunction(adapter$1) || (adapter$1 = adapter$1.get(config))))
      break;
...
var Axios$1 = class {
  ...
  _request(configOrUrl, config) {
    ...
    this.interceptors.request.forEach(function unshiftRequestInterceptors(interceptor) {
      ...
      if (transitional$1 && transitional$1.legacyInterceptorReqResOrdering)
